...
function Add-SmartArchiveBrowseEntry {
    param([hashtable]$Entries, [string]$RelativePath, [bool]$IsFolder)
    ...
    for ($i = 0; $i -lt $parts.Count; $i++) {
        ...
        if (-not $Entries.ContainsKey($key)) {
            $Entries[$key] = [pscustomobject]@{ Rel = $current; IsFolder = [bool]$isCurrentFolder }
        }
    ...
    try {
        ...
        foreach ($block in @($manifest.blocks)) {
            ...
            foreach ($entry in @(([string]$listResult.Output) -split "`r?`n")) {
                ...
                Add-SmartArchiveBrowseEntry $entries $clean $isFolder
            }
        ...
        foreach ($alias in @($manifest.dedupAliases)) {
            Add-SmartArchiveBrowseEntry $entries ([string]$alias.path) $false
        }

        $items = @($entries.Values | Sort-Object @{ Expression = { if ($_.IsFolder) { 0 } else { 1 } } }, @{ Expression = { [string]$_.Rel } })
        return [pscustomobject]@{ Work = $work; Manifest = $manifest; Entries = $items }
    }
    catch {
        Remove-SmartTarWorkAndRoot $work
        throw
...
function Add-SmartArchiveTreeNode {
    param([System.Windows.Forms.TreeView]$Tree, [hashtable]$NodeMap, [string]$RelativePath, [bool]$IsFolder)
    ...
    for ($i = 0; $i -lt $parts.Count; $i++) {
        ...
        if (-not $NodeMap.ContainsKey($key)) {
            ...
            $node.Tag = [pscustomobject]@{ Rel = $currentPath; IsFolder = [bool]$nodeIsFolder }
            if ($nodeIsFolder) {
                $node.ImageIndex = 0
...
function Show-SmartArchiveBrowseDialog {
    ...
    try {
        ...
        Set-AppStatus 'Ready.' ([System.Drawing.Color]::DimGray)
    }
    ...
    $lbl = New-EcoLabel ('Archive: ' + $ArchivePath) 12 10 740 20 $fItalic ([System.Drawing.Color]::DimGray)
    ...
    $rootNode = [System.Windows.Forms.TreeNode]::new([System.IO.Path]::GetFileName($ArchivePath))
    $rootNode.Tag = [pscustomobject]@{ Rel = ''; IsFolder = $true }
    ...
    try {
        foreach ($entry in @($browseData.Entries)) {
            Add-SmartArchiveTreeNode $tree $nodeMap ([string]$entry.Rel) ([bool]$entry.IsFolder)
        }
        $rootNode.Expand()
    }
    finally {
    ...
    $btnExtractSelected = New-EcoButton 'Extract selected...' 12 495 160 32 $fBold ([System.Drawing.Color]::SteelBlue) $cButtonText
    $btnClose = New-EcoButton 'Close' 592 495 160 32 $fBold ([System.Drawing.Color]::DarkSlateGray) $cButtonText
    ...
    $btnExtractSelected.Add_Click({
        ...
        try {
            ...
            if ($dialog.ShowDialog($browseForm) -eq [System.Windows.Forms.DialogResult]::OK) {
                ...
                [System.Windows.Forms.Cursor]::Current = [System.Windows.Forms.Cursors]::WaitCursor
                try {
                    $target = Extract-SmartArchiveSelection $TarPath $ArchivePath ([string]$tag.Rel) ([bool]$tag.IsFolder) $dialog.SelectedPath
...
function Show-Message {
    param(
        [string]$Message,
        [string]$Title = 'SmartTAR STAR v1.3.0',
...
function Show-ArchiveAddBrowseChoice {
    ...
    $pathLabel = New-EcoLabel $ArchivePath 16 40 430 45 $fItalic ([System.Drawing.Color]::DimGray)
    ...
    $btnAddChoice = New-EcoButton 'Add' 94 122 120 32 $fBold ([System.Drawing.Color]::SeaGreen) $cButtonText
    $btnBrowseChoice = New-EcoButton 'Browse' 256 122 120 32 $fBold ([System.Drawing.Color]::SteelBlue) $cButtonText
...
$form = New-UiObject 'System.Windows.Forms.Form' @{
    Text            = 'SmartTAR - STAR 1.3.0  .:: Copyright © 2026 eco-by-different ::.'
...
$lblInfo = New-EcoLabel '-=>> SmartTAR is an MTB project - Make TAR Better <<=-' 20 252 465 20 $fBold ([System.Drawing.Color]::DimGray)
