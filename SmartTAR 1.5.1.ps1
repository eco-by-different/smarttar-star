...
function Format-StatusLine {
    param([string]$Text)
    return ('[{0}] {1}' -f (Get-Date -Format 'HH:mm:ss'), ([string]$Text).Trim())
}

function Convert-StatusColorToText {
    param([System.Drawing.Color]$Color)
    return ('#{0:X2}{1:X2}{2:X2}' -f $Color.R, $Color.G, $Color.B)
}

function Add-ColoredOutputLine {
    param([string]$Text, [System.Drawing.Color]$Color = $cTextMuted)
    if ($null -eq $txtOutput -or (Test-Blank $Text)) { return }
    $txtOutput.SelectionStart = $txtOutput.TextLength
    $txtOutput.SelectionLength = 0
    $txtOutput.SelectionColor = $Color
    $txtOutput.AppendText($Text + [Environment]::NewLine)
    $txtOutput.SelectionColor = $cTextMuted
}

function Add-StatusRecordsToOutput {
    param([string]$Records)
    if ($null -eq $txtOutput -or (Test-Blank $Records)) { return }
    foreach ($record in @($Records -split '\r?\n')) {
        if (Test-Blank $record) { continue }
        $parts = $record.Split([char]9, 2)
        $color = $cTextMuted
        $text = $record
        if ($parts.Count -eq 2 -and $parts[0] -match '^#[0-9A-Fa-f]{6}$') {
            try { $color = [System.Drawing.ColorTranslator]::FromHtml($parts[0]) } catch { $color = $cTextMuted }
            $text = $parts[1]
        }
        Add-ColoredOutputLine $text $color
    }
    $txtOutput.SelectionStart = $txtOutput.TextLength
    $txtOutput.SelectionLength = 0
...
function Write-WorkerStatusFile {
    param([string]$Path, [string]$Text, [System.Drawing.Color]$Color = $cTextMuted)
    if ((Test-Blank $Path) -or (Test-Blank $Text)) { return }
    $dir = Split-Path -Parent $Path
    if (-not (Test-Blank $dir)) { [System.IO.Directory]::CreateDirectory($dir) | Out-Null }
    $record = (Convert-StatusColorToText $Color) + [char]9 + (Format-StatusLine $Text) + [Environment]::NewLine
    for ($n = 0; $n -lt 10; $n++) {
        try {
            [System.IO.File]::AppendAllText($Path, $record, [System.Text.Encoding]::UTF8)
            return
        }
...
function Set-AppStatus {
    ...
    if ($script:workerConfig -and -not (Test-Blank ([string]$script:workerConfig.StatusFile))) {
        Write-WorkerStatusFile ([string]$script:workerConfig.StatusFile) $Text $Color
        return
    }

    if ($null -ne $txtOutput -and -not (Test-Blank $Text)) {
        Add-ColoredOutputLine (Format-StatusLine $Text) $Color
        $txtOutput.SelectionStart = $txtOutput.TextLength
        $txtOutput.SelectionLength = 0
        $txtOutput.ScrollToCaret()
        try { $txtOutput.Refresh() } catch {}
    }
}
...
function Merge-StarAddManifest {
    ...
    $result=[ordered]@{
        format='STAR';formatVersion=2;layout='multi-root-additive';tool='SmartTAR';toolVersion='1.5.1';createdUtc=(Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
...
if (-not (Test-Blank $WorkerConfigFile)) {
    try {
        ...
        try { Set-AppStatus ("Operation failed: " + [string]$_.Exception.Message) $cDanger } catch {}
        try {
...
function Start-WorkerOperation {
    ...
    if ($null -ne $txtOutput) { $txtOutput.Clear() }
    [System.IO.File]::WriteAllText($script:currentStatusFile, '', [System.Text.Encoding]::UTF8)
    Write-WorkerStatusFile $script:currentStatusFile "$Action started..."
    if ($null -ne $txtOutput) {
        $initialStatus = ''
        try { $initialStatus = Get-Content -LiteralPath $script:currentStatusFile -Raw -Encoding UTF8 } catch {}
        Add-StatusRecordsToOutput $initialStatus
        $script:lastStatusSnapshot = $initialStatus
    }
...
function Show-Message {
    param(
        [string]$Message,
        [string]$Title = 'SmartTAR STAR v1.5.1',
...
$form = [System.Windows.Forms.Form]@{
    Text            = 'SmartTAR - STAR 1.5.1  .:: Copyright © 2026 eco-by-different ::.'
...
$chkOpenFolder  = New-EcoCheck 'Open output folder after success' 20 342 245 $true
...
$chkSalvageMode = New-EcoCheck 'Salvage mode (Ignore broken blocks)' 0 342 240 $false
$chkSalvageMode.AutoSize = $true
# WinForms CheckBox keeps a small right-side text padding. Anchor the control
# 13 px beyond the VERIFY edge so the visible word 'mode' ends below the button.
$chkSalvageMode.Location = New-Point (500 - $chkSalvageMode.PreferredSize.Width) 342
...
Add-ColoredOutputLine (Format-StatusLine 'Ready.') $cTextMuted
$script:lastStatusSnapshot = ''
...
$timer.Add_Tick({
    try {
        if (-not (Test-Blank $script:currentStatusFile) -and (Test-Path -LiteralPath $script:currentStatusFile)) {
            $statusText = ''
            try { $statusText = Get-Content -LiteralPath $script:currentStatusFile -Raw -Encoding UTF8 -ErrorAction Stop } catch {}
            if (-not (Test-Blank $statusText) -and $script:lastStatusSnapshot -ne $statusText) {
                if (-not (Test-Blank $script:lastStatusSnapshot) -and $statusText.StartsWith($script:lastStatusSnapshot, [System.StringComparison]::Ordinal)) {
                    $newText = $statusText.Substring($script:lastStatusSnapshot.Length)
                    if ($newText.Length -gt 0) { Add-StatusRecordsToOutput $newText }
                }
                else {
                    $txtOutput.Clear()
                    Add-StatusRecordsToOutput $statusText
                }
                $script:lastStatusSnapshot = $statusText
            }
        }
        ...
        if ($script:currentProcess -and $script:currentProcess.HasExited) {
            ...
            if ($completion.Success) {
                Set-AppStatus 'Operation completed successfully.' $cStatusOk
                if (Test-Blank $shownReport) {
                    Set-AppStatus 'Report path is unavailable.' $cWarning
                }
                else {
                    Set-AppStatus "Report saved to: $shownReport" $cTextMuted
                }
                ...
                Set-AppStatus 'Operation failed.' $cDanger
                if (Test-Blank $shownReport) {
                    Set-AppStatus 'The error report could not be saved.' $cDanger
                }
                else {
                    Set-AppStatus "Error report saved to: $shownReport" $cWarning
                }
            }
...
$form.Add_FormClosing({
    ...
    try {
        ...
        if ($disposeUiResources -and -not $_.Cancel) {
            ...
            try { $txtOutput.Font.Dispose() } catch {}
        }
    }
})
