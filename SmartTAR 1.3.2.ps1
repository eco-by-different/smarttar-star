...
function Add-SmartArchiveTreeNode {
    ...
    $rel = (Convert-ToTarPath $RelativePath).Trim('/').Trim()
    if (Test-Blank $rel) { return }
    $root = (Convert-ToTarPath $ArchiveRoot).Trim('/').Trim()
    $parts = @($rel.Split('/') | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    $currentPath = ''
    $parentNodes = $Tree.Nodes[0].Nodes

    for ($i = 0; $i -lt $parts.Count; $i++) {
        $part = [string]$parts[$i]
        $currentPath = if (Test-Blank $currentPath) { $part } else { $currentPath + '/' + $part }
        $archivePath = if (Test-Blank $root) { $currentPath } else { $root + '/' + $currentPath }
        $key = $archivePath.ToLowerInvariant()
        $nodeIsFolder = if ($i -lt ($parts.Count - 1)) { $true } else { [bool]$IsFolder }

        if (-not $NodeMap.ContainsKey($key)) {
            $node = [System.Windows.Forms.TreeNode]::new($part)
            $node.Tag = [pscustomobject]@{ Rel = $archivePath; IsFolder = [bool]$nodeIsFolder }
            if ($nodeIsFolder) {
                $node.ImageIndex = 0
            ...
            [void]$parentNodes.Add($node)
            $NodeMap[$key] = $node
        }
        else {
            $node = $NodeMap[$key]
            if ($nodeIsFolder -and $null -ne $node.Tag) { $node.Tag.IsFolder = $true }
        }
...
function Extract-SmartArchiveSelection {
    ...
    $rel = (Convert-ToTarPath ([string]$RelativePath)).Trim('/').Trim()
    # Both Browse roots represent the complete archive:
    # 1) the artificial archive node (empty Rel),
    # 2) the stored source root from manifest.sourceName.
    # Keep both on the proven full extraction pipeline.
    $sourceRoot = ''
    if ($null -ne $BrowseData -and $null -ne $BrowseData.Manifest) {
        $sourceRoot = (Convert-ToTarPath ([string]$BrowseData.Manifest.sourceName)).Trim('/').Trim()
    }
    if ((Test-Blank $rel) -or
        (-not (Test-Blank $sourceRoot) -and $rel.Equals($sourceRoot, [System.StringComparison]::OrdinalIgnoreCase))) {
        return (Extract-SmartArchiveSelectionLegacy $TarPath $ArchivePath $RelativePath $IsFolder $DestinationParent)
    }
    if ($null -eq $BrowseData -or $null -eq $BrowseData.EntryMap) {
        return (Extract-SmartArchiveSelectionLegacy $TarPath $ArchivePath $RelativePath $IsFolder $DestinationParent)
    }

    $entryKey = $rel.ToLowerInvariant()
    if (-not $BrowseData.EntryMap.ContainsKey($entryKey)) {
        return (Extract-SmartArchiveSelectionLegacy $TarPath $ArchivePath $RelativePath $IsFolder $DestinationParent)
    }
    ...
    $entry = $BrowseData.EntryMap[$entryKey]
    $neededBlocks = @($entry.Blocks)
    if ($neededBlocks.Count -lt 1) {
        return (Extract-SmartArchiveSelectionLegacy $TarPath $ArchivePath $RelativePath $IsFolder $DestinationParent)
    }

    $work = New-SafeWorkRoot 'browse_stream' $ArchivePath
    $payload = Join-Path $work 'payload'
    [System.IO.Directory]::CreateDirectory($payload) | Out-Null

    try {
        $manifest = $BrowseData.Manifest
        $blockMap = @{}
        foreach ($block in @($manifest.blocks)) {
            $blockMap[(Convert-ToTarPath ([string]$block.path)).ToLowerInvariant()] = $block
        }

        foreach ($relativeBlock in $neededBlocks) {
            $blockKey = (Convert-ToTarPath ([string]$relativeBlock)).ToLowerInvariant()
            if (-not $blockMap.ContainsKey($blockKey)) { throw "Indexed block was not found in manifest: $relativeBlock" }
            $block = $blockMap[$blockKey]
            Set-BusyStatus "Streaming selected item from block $($block.id) $($block.group)..."
            [void](Invoke-SmartTarStreamSelection $TarPath $ArchivePath ([string]$block.path) ([string]$block.sha256) $payload $rel $IsFolder)
        }

        # A selected dedup alias may point outside the selected folder. Stream its
        # physical target separately, then recreate only aliases in the selection.
        $extraTargets = @{}
        foreach ($alias in @($manifest.dedupAliases)) {
            $aliasPath = (Convert-ToTarPath ([string]$alias.path)).Trim('/').Trim()
            $wanted = if ($IsFolder) {
                $aliasPath -eq $rel -or $aliasPath.StartsWith($rel + '/', [System.StringComparison]::OrdinalIgnoreCase)
            }
            else { $aliasPath -eq $rel }
            if (-not $wanted) { continue }

            $target = (Convert-ToTarPath ([string]$alias.target)).Trim('/').Trim()
            if ($target.StartsWith($rel + '/', [System.StringComparison]::OrdinalIgnoreCase) -or $target -eq $rel) { continue }
            ...
            foreach ($targetBlock in @($BrowseData.EntryMap[$targetKey].Blocks)) {
                $extraTargets[[string]$targetBlock + '|' + $target] = [pscustomobject]@{ Block = [string]$targetBlock; Target = $target }
            }
        }
        foreach ($extra in $extraTargets.Values) {
            $blockKey = (Convert-ToTarPath ([string]$extra.Block)).ToLowerInvariant()
            $block = $blockMap[$blockKey]
            Set-BusyStatus "Streaming dedup source from block $($block.id) $($block.group)..."
            [void](Invoke-SmartTarStreamSelection $TarPath $ArchivePath ([string]$block.path) ([string]$block.sha256) $payload ([string]$extra.Target) $false)
        }

        Restore-SelectedDedupAliases $manifest $payload $rel $IsFolder
        $sourceItem = Get-SafePayloadPath $payload $rel
        if (-not (Test-Path -LiteralPath $sourceItem)) { throw "Selected item was not restored from archive: $rel" }
        ...
        # Compatibility fallback: if the local bsdtar build rejects streamed
        # nested extraction, use the original disk-backed Browse pipeline.
...
function Show-SmartArchiveBrowseDialog {
    ...
    $sourceRoot = (Convert-ToTarPath ([string]$browseData.Manifest.sourceName)).Trim('/').Trim()
    if (Test-Blank $sourceRoot) { $sourceRoot = Get-ArchiveBaseNameWithoutSmartExtension $ArchivePath }
    $rootNode = [System.Windows.Forms.TreeNode]::new($sourceRoot)
    $rootNode.Tag = [pscustomobject]@{ Rel = $sourceRoot; IsFolder = $true }
    [void]$tree.Nodes.Add($rootNode)
    $nodeMap = @{}
    $nodeMap[$sourceRoot.ToLowerInvariant()] = $rootNode

    $tree.BeginUpdate()
    try {
        foreach ($entry in @($browseData.Entries)) {
            $entryRel = (Convert-ToTarPath ([string]$entry.Rel)).Trim('/').Trim()
            if ($entryRel.Equals($sourceRoot, [System.StringComparison]::OrdinalIgnoreCase)) { continue }
            if ($entryRel.StartsWith($sourceRoot + '/', [System.StringComparison]::OrdinalIgnoreCase)) {
                $displayRel = $entryRel.Substring($sourceRoot.Length + 1)
                Add-SmartArchiveTreeNode $tree $nodeMap $displayRel ([bool]$entry.IsFolder) $sourceRoot
            }
            else {
                # Single-file archives or unusual legacy roots are still shown safely.
                Add-SmartArchiveTreeNode $tree $nodeMap $entryRel ([bool]$entry.IsFolder) ''
            }
        }
        $rootNode.Expand()
    }
...
function Show-Message {
    param(
        [string]$Message,
        [string]$Title = 'SmartTAR STAR v1.3.2',
    ...
    try {
        ...
        if ($script:currentProcess -and $script:currentProcess.HasExited) {
            ...
            if ($completion.Success) {
                Set-AppStatus "$($completion.Action) completed successfully." $cStatusOk
                ...
                Set-AppStatus "$($script:currentAction) failed." $cDanger
