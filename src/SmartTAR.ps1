...
function Get-SortedSourceFiles {
    ...
    if (Is-Blank $BaseRoot) {
        return @(Get-ChildItem -LiteralPath $Source -File -Recurse -Force -ErrorAction SilentlyContinue | Sort-Object @{Expression={$_.FullName.ToLowerInvariant()}},@{Expression={$_.FullName}})
    }
...
function New-ArchiveGroups {
    ...
    switch ($Mode) {
        "Solid" {
            ...
            $groups["solid"] = New-GroupInfo "solid" $solid (Join-Path $StagingRoot "solid") "Auto solid method selected from source profile."
        }
        "SmartXZ" {
...
function Test-GroupUsesXz {
    param($Group)
    try { return ([string]$Group.Method.Algorithm -eq "xz") } catch { return $false }
}
...
function Set-XzGroupTimestamps {
    ...
    foreach ($key in $Groups.Keys) {
        if (Test-GroupUsesXz $Groups[$key]) { Set-TreeTimestamp $Groups[$key].Stage $fixedTime }
    }
}
...
function Build-Manifest {
    ...
    return [ordered]@{
        ...
        planning        = [ordered]@{
            ...
            design            = "Transparent TAR outer container with SmartTAR block manifest."
        }
        ...
        manualRecovery = @(
            ...
            "If your tool does not recognize .star, rename archive.star to archive.tar and inspect it manually."
        )
    }
}
...
function Extract-Blocks {
    ...
    Set-AppStatus "Extracting internal blocks..." ([System.Drawing.Color]::DarkOrange)
    foreach ($block in $Blocks) {
        ...
        Invoke-Tar $TarPath @("-xf",$blockPath,"-C",$DestinationFolder) "Block extraction failed: $relativePath."
    }
}
...
function Get-UsedMethodReport {
    ...
    $zstdBlocks = @(@($Blocks) | Where-Object { ([string]$_.compression -eq "zstd") -or ([string]$_.method -like "zstd*") })
    $configured = "n/a"
    try { if ($Manifest.planning -and ($Manifest.planning.PSObject.Properties.Name -contains "zstdMaxLevel")) { $configured = [string]$Manifest.planning.zstdMaxLevel } } catch {}
    $zstd = if ($zstdBlocks.Count -gt 0) { "used in $($zstdBlocks.Count) block(s), configured max level: $configured" } else { "not used" }
...
function Verify-SmartArchive {
    ...
    try {
        ...
        Set-AppStatus "Verifying internal blocks..." ([System.Drawing.Color]::DarkOrange)
        foreach ($block in $blocks) {
            $relativePath = [string]$block.path
            $blockPath = Resolve-SafeBlockPath $outer $relativePath
            if (-not (Test-Path -LiteralPath $blockPath)) {
                $fail++
                $lines += ("  {0} | {1} | {2} | MISSING | {3}" -f $block.id,$block.group,$block.display,$relativePath)
                continue
            }
            ...
            if ($listed -and $hashOk) { $status="OK" } elseif ($listed -and -not $hashOk) { $status="HASH FAIL" } else { $status="FAIL" }
            $sizeInfo = Format-Bytes $actualSize
            if ($expectedSize -gt 0 -and $expectedSize -ne $actualSize) { $sizeInfo = "$sizeInfo / expected $(Format-Bytes $expectedSize)" }
            $hashInfo = ""
            if (-not (Is-Blank $expectedHash)) { $hashInfo = if ($hashOk) { " | SHA256 OK" } else { " | SHA256 FAIL" } }
            $reason = [string]$block.reason
            if (Is-Blank $reason) { $reason = "n/a" }
            $lines += ("  {0} | {1} | {2} | {3} | {4}{5} | {6}" -f $block.id,$block.group,$block.display,$status,$sizeInfo,$hashInfo,$reason)
        }

        $verificationStatus = if ($fail -gt 0) { "Archive verification PARTIAL / FAILED (Some blocks failed, healthy blocks may still be recoverable)" } else { "Archive verification OK (Integrity verified)" }
...
function Set-DefaultTarget {
    ...
    if ($script:selectedType -eq "File" -and (Is-SmartArchivePath $script:selectedPath)) {
        $txtTarget.Text = Join-Path $parent ((Get-DefaultExtractBaseName $script:selectedPath) + "_extracted")
        return
    }
...
$lblTarget = New-EcoLabel "2. Destination archive / extraction folder:" 20 125 -Font $fBold
...
$lblInfo      = New-EcoLabel "XZ/XZ9 blocks use fixed timestamps; STORE and ZSTD blocks keep natural timestamps" 20 252 465 20 $fItalic ([System.Drawing.Color]::DimGray)
...
$cmbMode.Add_SelectedIndexChanged({
    ...
    if ($mode -eq "Solid") {
        $lblInfo.Text = "Solid = one block; XZ selections use XZ9 and fixed timestamps"
    } elseif ($mode -eq "SmartXZ") {
        $lblInfo.Text = "Smart XZ = grouped XZ9 blocks with fixed timestamps; media/archive STORE"
    } elseif ($mode -eq "Smart") {
        $lblInfo.Text = "Smart = XZ parts use XZ9/fixed timestamps; binary/exe/diskimage prefer ZSTD19"
    } else {
        $lblInfo.Text = "Hybrid = compressible XZ9/fixed timestamps, disk images ZSTD19, media/archive STORE"
    }
})
...
$btnFile.Add_Click({
    ...
    try { if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { Set-SelectedPath $dialog.FileName "File" } }
...
$btnFolder.Add_Click({
    ...
    try { if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { Set-SelectedPath $dialog.SelectedPath "Folder" } }
    finally { $dialog.Dispose() }
})
...
$btnTarget.Add_Click({
    if ($script:selectedType -eq "File" -and (Is-SmartArchivePath $script:selectedPath)) {
        ...
        try { if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { $txtTarget.Text = $dialog.SelectedPath } }
        finally { $dialog.Dispose() }
        return
    }
    ...
    try { if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { $txtTarget.Text = Ensure-StarExtension $dialog.FileName } }
    finally { $dialog.Dispose() }
})
...
function Execute-Compress {
    ...
    if ($success) {
        ...
        try {
            ...
            $fallback = "Archive created successfully, but automatic summary/verify failed.`r`n`r`nArchive: $targetPath`r`nMode: $mode`r`nSource: $($script:selectedPath)`r`nArchive size: $(Format-Bytes ([int64](Get-Item -LiteralPath $targetPath).Length))`r`n`r`nVerify/report error:`r`n$verifyError"
            [void](Write-ReportFile $reportPath $fallback)
            Msg "$fallback`r`nReport saved:`r`n$reportPath" "Archive Created - Verify Report Failed" ([System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null
        }
...
function Execute-Extract {
    ...
    if (Is-Blank $destination) { Msg "Please select extraction folder." "Missing Destination" ([System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null; return }
    ...
    if ($success) {
        ...
        $text = "Archive extracted successfully.`r`nArchive: $($script:selectedPath)`r`nDestination: $destination"
...
function Execute-Verify {
    ...
    if ($success) {
        [void](Write-ReportFile $reportPath $summary)
        Set-AppStatus "Archive verification finished." ([System.Drawing.Color]::Green)
