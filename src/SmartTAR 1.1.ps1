...
function Set-UiBusy {
    ...
    foreach ($control in @($btnFile, $btnFolder, $btnArchive, $btnTarget, $cmbMode, $chkOpenFolder, $chkAdaptive, $chkSalvageMode)) {
...
function New-AdaptiveStats { return [ordered]@{ enabled=[bool]$script:adaptiveDeepAnalyze; scope='unknown-files-only'; method='magic-bytes-plus-conservative-byte-entropy-start-end-sample'; sampleBytes=[int]$script:AdaptiveSampleBytes; unknownSeen=0; unknownBytes=[int64]0; movedToText=0; movedToTextBytes=[int64]0; movedToBinary=0; movedToBinaryBytes=[int64]0; movedToArchives=0; movedToArchivesBytes=[int64]0; stayedUnknown=0; stayedUnknownBytes=[int64]0; errors=0 } }
function Add-AdaptiveDecisionStat { param([string]$Decision,[int64]$Bytes,[bool]$Error=$false); if ($null -eq $script:adaptiveStats) { $script:adaptiveStats = New-AdaptiveStats }; $script:adaptiveStats.unknownSeen=[int]$script:adaptiveStats.unknownSeen+1; $script:adaptiveStats.unknownBytes=[int64]$script:adaptiveStats.unknownBytes+$Bytes; if ($Error) { $script:adaptiveStats.errors=[int]$script:adaptiveStats.errors+1 }; switch([string]$Decision){'text'{$script:adaptiveStats.movedToText=[int]$script:adaptiveStats.movedToText+1;$script:adaptiveStats.movedToTextBytes=[int64]$script:adaptiveStats.movedToTextBytes+$Bytes}'binary'{$script:adaptiveStats.movedToBinary=[int]$script:adaptiveStats.movedToBinary+1;$script:adaptiveStats.movedToBinaryBytes=[int64]$script:adaptiveStats.movedToBinaryBytes+$Bytes}'archives'{$script:adaptiveStats.movedToArchives=[int]$script:adaptiveStats.movedToArchives+1;$script:adaptiveStats.movedToArchivesBytes=[int64]$script:adaptiveStats.movedToArchivesBytes+$Bytes}default{$script:adaptiveStats.stayedUnknown=[int]$script:adaptiveStats.stayedUnknown+1;$script:adaptiveStats.stayedUnknownBytes=[int64]$script:adaptiveStats.stayedUnknownBytes+$Bytes}} }
...
function Stage-FilesPlan {
    ...
    foreach ($file in (Get-SortedSourceFiles $SourceItem $Source $BaseRoot)) {
        ...
        if ($script:adaptiveDeepAnalyze -and $smartGroup -eq 'unknown') {
            $adaptiveGroup = 'unknown'; $adaptiveError = $false
            try { $adaptiveGroup = Get-AdaptiveSmartGroupName $file; if (Test-Blank $adaptiveGroup) { $adaptiveGroup = 'unknown' } }
            catch { $adaptiveGroup = 'unknown'; $adaptiveError = $true }
            Add-AdaptiveDecisionStat $adaptiveGroup ([int64]$file.Length) $adaptiveError
...
function Build-Blocks {
    ...
    if ($StructureDirCount -gt 0) { $id = '{0:D6}' -f $index; $blockPath = Join-Path $BlocksDir ("$id`_structure.tar"); Set-AppStatus "Creating block $id structure..." ([System.Drawing.Color]::DarkOrange); Create-BlockFromStageDirect $TarPath $StructureStage $blockPath $StoreMethod; Add-BlockManifestItem ([ref]$blocks) $id 'structure' $blockPath $StoreMethod 'Directory structure only.' 0 $StructureDirCount 0; $index++ }
...
function Format-AdaptiveDiagnostics {
    param($Manifest)
    try { $diag = $Manifest.adaptiveDiagnostics; if ($null -eq $diag) { return '' }; if (-not ([bool]$diag.enabled)) { return "`r`n`r`nAdaptive deep analyze: OFF" }; $lines = @('', 'Adaptive deep analyze: ON'); $lines += ('Unknown analyzed: {0}, source={1}' -f ([int]$diag.unknownSeen), (Format-Bytes ([int64]$diag.unknownBytes))); $lines += ('Moved to text group: {0}, source={1}' -f ([int]$diag.movedToText), (Format-Bytes ([int64]$diag.movedToTextBytes))); $lines += ('Moved to binary group: {0}, source={1}' -f ([int]$diag.movedToBinary), (Format-Bytes ([int64]$diag.movedToBinaryBytes))); $lines += ('Moved to STORE group: {0}, source={1}' -f ([int]$diag.movedToArchives), (Format-Bytes ([int64]$diag.movedToArchivesBytes))); $lines += ('Stayed unknown: {0}, source={1}' -f ([int]$diag.stayedUnknown), (Format-Bytes ([int64]$diag.stayedUnknownBytes))); $lines += ('Errors: {0}' -f ([int]$diag.errors)); return ($lines -join "`r`n") } catch { return '' }
}
...
function Verify-SmartArchive {
    ...
    $outer = Join-Path $work 'outer'
    [System.IO.Directory]::CreateDirectory($outer) | Out-Null
    try { $safeArchive = Prepare-SafeArchiveInput $ArchivePath $work; Invoke-Tar $TarPath @('-xf', $safeArchive, '-C', $outer) 'Outer verification failed.'; $manifest = Read-OuterManifest $outer; $blocks = @($manifest.blocks); $ok = 0; $fail = 0; $lines = @(); foreach ($block in $blocks) { Set-AppStatus "Verifying block $($block.id) $($block.group)..." ([System.Drawing.Color]::DarkOrange); $blockPath = Resolve-SafeBlockPath $outer ([string]$block.path); if (-not (Test-Path -LiteralPath $blockPath)) { $fail++; $lines += "MISSING: $($block.path)"; continue }; $listed = Invoke-TarList $TarPath $blockPath; $hashOk = $true; if ($block.sha256) { $hashOk = ((Get-FileSHA256 $blockPath) -eq ([string]$block.sha256).ToLowerInvariant()) }; if ($listed -and $hashOk) { $ok++ } else { $fail++; $lines += "FAIL: $($block.id) $($block.group) $($block.path)" } }; $verification = if ($fail -eq 0) { 'OK' } else { 'FAILED' }; $diag = Format-GroupDiagnostics $manifest; $adaptiveDiag = Format-AdaptiveDiagnostics $manifest; $failedDetails = ''; if ($fail -gt 0 -and $lines.Count -gt 0) { $failedDetails = "`r`n`r`nFailed block details:`r`n" + ($lines -join "`r`n") }; return @"
Archive:
...
Blocks OK: $ok
Blocks failed: $fail
Verification: $verification$diag$adaptiveDiag$failedDetails
...
if (-not (Test-Blank $WorkerConfigFile)) {
    try {
        ...
        $script:adaptiveDeepAnalyze = [bool]$cfg.AdaptiveDeepAnalyze
...
function Start-WorkerOperation {
    ...
    $script:adaptiveDeepAnalyze = [bool]$AdaptiveDeepAnalyze
    $script:currentStdOut = ''
    $script:currentStdErr = ''
$script:ToolVersion = '1.1'
$script:FormatName = 'STAR'
...
$script:adaptiveStats = $null
    ...
    $config = [ordered]@{
        ...
        Mode               = $Mode
        Salvage            = $Salvage
        AdaptiveDeepAnalyze = [bool]$AdaptiveDeepAnalyze
...
function Show-Message {
    param(
        [string]$Message,
        [string]$Title = 'SmartTAR STAR v1.1',
...
$form = New-UiObject 'System.Windows.Forms.Form' @{
    Text            = 'SmartTAR - STAR v1.1  ..:: Copyright (c) 2026 eco-by-different ::..'
...
$lblInfo = New-EcoLabel 'STAR v1.1 stable build.' 20 252 465 20 $fItalic ([System.Drawing.Color]::DimGray)
...
$chkAdaptive    = New-EcoCheck 'Adaptive - deep analyze' 290 342 200 $false
...
$form.Controls.AddRange([System.Windows.Forms.Control[]]@(
    ...
    $chkOpenFolder, $chkAdaptive, $chkSalvageMode,
...
function Execute-Compress {
    ...
    Start-WorkerOperation 'Compress' $script:selectedPath $targetPath (Get-SelectedCompressionMode) $false ([bool]$chkAdaptive.Checked)
}
...
function Execute-Extract {
    ...
    Start-WorkerOperation 'Extract' $script:selectedPath $destination 'Hybrid' ([bool]$chkSalvageMode.Checked) $false
}
...
function Execute-Verify {
    ...
    Start-WorkerOperation 'Verify' $script:selectedPath $txtTarget.Text 'Hybrid' $false $false
}
